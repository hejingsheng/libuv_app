...
{
public :
    TcpConnection(EventLoop* loop,std::string& name,UVTcpPtr client, bool istls, bool isConnected = true);
    virtual ~TcpConnection();

	int init(std::string cert, std::string key, TLSRole role);
	...
	void writeTls(const char* buf, ssize_t size, AfterWriteCallback callback);
...
private:
	int initTlsServer(std::string cert, std::string key);
	int initTlsClient();
	void send_bio_data();
